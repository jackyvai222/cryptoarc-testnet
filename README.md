...
* **`PaymentReceived(address indexed user, uint256 amount, uint256 timestamp)`**
  Emitted whenever a user successfully purchases premium access.
* **`PriceChanged(uint256 oldPrice, uint256 newPrice)`**
  Emitted when the contract owner updates the premium membership price.
